...
- **Q: 端口不是3000/5173？**
  - A: 本项目每次启动会自动分配一个5位端口，终端和页面会显示访问地址。
- **Q: 手机能访问吗？**
...
def post_message():
    ...
    # ...保存消息逻辑...
    return jsonify({'message': '消息保存成功', 'text': msg})
```
...
sequenceDiagram
    participant 用户
    participant 前端(FilePanel.vue/api/file.ts)
    participant 后端(file.py)
    用户->>前端: 选择文件并点击上传
    前端->>后端: POST /api/file/upload (FormData)
    后端-->>前端: 返回上传结果(JSON)
    前端-->>用户: 刷新文件列表，显示新文件
```
...
sequenceDiagram
    participant 用户
    participant 前端(MessagePanel.vue/api/message.ts)
    participant 后端(message.py)
    用户->>前端: 输入消息并点击发送
    前端->>后端: POST /api/message/ (JSON)
    后端-->>前端: 返回保存结果(JSON)
    前端-->>用户: 刷新消息区，显示新消息
```
...
graph TD
  A[用户浏览器] 
  B[前端 Vue3/Vite]
  C[API 封装 (api/message.ts, api/file.ts, api/video.ts)]
  D[后端 Flask]
  E[消息API message.py]
  F[文件API file.py]
  G[视频API video.py]
...
## 12. 典型功能流程图（以"文件上传"为例）

```mermaid
sequenceDiagram
    participant 用户
    participant 前端(FilePanel.vue)
    participant API(api/file.ts)
    participant 后端(file.py)
    participant 存储(uploads/)

    用户->>前端: 选择文件并点击上传
    前端->>API: 调用uploadFile(file)
    API->>后端: POST /api/file/upload (FormData)
    后端->>存储: 保存文件
    后端-->>API: 返回上传结果(JSON)
    API-->>前端: Promise resolve
    前端-->>用户: 刷新文件列表，显示新文件
```
...
graph LR
  App[App.vue]
  Msg[MessagePanel.vue]
  File[FilePanel.vue]
  Video[VideoPanel.vue]
  Set[SettingsDialog.vue]
  AM[api/message.ts]
  AF[api/file.ts]
  AV[api/video.ts]
...
如需更多代码示例、流程说明或结构图，欢迎查阅源码或联系开发者！ 